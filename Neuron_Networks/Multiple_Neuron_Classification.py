...
def multiple_neuron_classification(input_cols, target_col, eta, iteration):    
    ...
    for i in range(iteration):
        ...
        for neuron in range(W.shape[1]):
            neuron_name = "Neuron_" + str(neuron)
            # update weights
            for weight in range(W.shape[0] - 1):
                weight_name = "W" + str(weight)
...
# invoke function
weight_variation = multiple_neuron_classification(df[df.columns[:4]], df[df.columns[4]], 0.1, 1000)

# visualize weights variation
line_style = ["-", "--", "-.", ":"]
color_platte = ["r", "g", "b"]
